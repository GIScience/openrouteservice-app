angular.module('orsApp.locations-service', [])
    .factory('orsLocationsService', ['$q', '$http', 'orsUtilsService', 'orsMapFactory', 'orsObjectsFactory', 'lists', 'ENV', ($q, $http, orsUtilsService, orsMapFactory, orsObjectsFactory, lists, ENV) => {
        /**
        ...
        orsLocationsService.panTo = (geometry) => {
            let action = orsObjectsFactory.createMapAction(0, lists.layers[2], {
                lat: geometry[1],
                lng: geometry[0]
            }, undefined, undefined);
        ...
        orsLocationsService.emphPoi = (geometry, category) => {
            let action = orsObjectsFactory.createMapAction(11, lists.layers[2], {
                lat: geometry[1],
                lng: geometry[0]
            }, undefined, category);
            orsMapFactory.mapServiceSubject.onNext(action);
