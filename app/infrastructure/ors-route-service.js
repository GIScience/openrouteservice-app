angular.module('orsApp.route-service', []).factory('orsRouteService', ['$q', '$http', 'orsUtilsService', 'orsMapFactory', 'orsObjectsFactory', ($q, $http, orsUtilsService, orsMapFactory, orsObjectsFactory) => {
    ...
    orsRouteService.parseLinestring = (routeGeometry) => {
        let routeString = [];
        _.each(orsUtilsService.getElementsByTagNameNS(routeGeometry, namespaces.gml, 'pos'), (point, i) => {
    ...
    orsRouteService.parseSummary = (summary) => {
        ...
        let boundingBoxXML = orsUtilsService.getElementsByTagNameNS(summary, namespaces.xls, 'BoundingBox')[0],
            boundingBox = [];
        _.each(orsUtilsService.getElementsByTagNameNS(boundingBoxXML, namespaces.gml, 'pos'), (point, i) => {
            point = point.text || point.textContent;
            point = point.split(' ');
            point = [point[1], point[0]];
