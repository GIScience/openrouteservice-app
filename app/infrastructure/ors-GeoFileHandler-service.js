            ...
            orsWriterFactory.writeGpx = function(route, options, avgSpeed, coordinatePrecision) {
                ...
                var gpx = {
                    "gpx": {
                        ...
                        "trk": [],
                        "$version": "1.1",
                        "$creator": namespaces.metadata.authorName,
                        "$xmlns:gpx": namespaces.gpx,
                        "$xmlns:xsi": namespaces.xsi,
                        "$xsi:schemaLocation": namespaces.schemata.gpxService,
                    },
                };
                // create the metadata object
                var metadata = {
                    "name": namespaces.metadata.name,
                    "desc": namespaces.metadata.description,
                    "author": {
                        "name": namespaces.metadata.authorName,
                        "email": {
                            "$id": namespaces.metadata.authorEmailId,
                            "$domain": namespaces.metadata.authorEmailDomain,
                        },
                        "link": {
                            "$href": namespaces.metadata.link,
                        },
                    },
                    "copyright": {
                        "year": date.year,
                        "license": namespaces.metadata.license,
                        "$author": namespaces.metadata.authorName,
                    },
                    "link": {
                        "$href": namespaces.metadata.link,
                    },
                    "time": date.toISOString(),
                    "keywords": namespaces.metadata.keywords,
                    "bounds": {
                ...
                var rte = {
                    ...
                    "link": {
                        "text": namespaces.metadata.authorName,
                        "$href": namespaces.metadata.link,
                    },
                    "rtept": [],
                };
            ...
            orsWriterFactory.writeTcx = function(route, options, avgSpeed, coordinatePrecision) {
                ...
                var tcx = {
                    "TrainingCenterDatabase": {
                        "Folders": [],
                        "Courses": [],
                        "$xmlns:tcx": namespaces.tcx,
                        "$xmlns:xsi": namespaces.xsi,
                        "$xsi:schemaLocation": namespaces.schemata.tcxService,
                    },
                };
            ...
            orsWriterFactory.writeKml = function(route, options, avgSpeed, coordinatePrecision) {
                ...
                var kml = {
                    "kml": {
                        "Document": {
                            "name": namespaces.metadata.name,
                            "description": namespaces.metadata.description + "\nAuthor: " + namespaces.metadata.authorName + "\nAuthor eMail: " + namespaces.metadata.authorEmailId + "@" + namespaces.metadata.authorEmailDomain + "\nAuthor Link: " + namespaces.metadata.link + "\nCopyright: " + namespaces.metadata.copyright + "\nDate: " + date.toISOString() + "\nKeywords:" + namespaces.metadata.keywords,
                            //"Style":[],
                            //"StyleMap":[],
                            "Placemark": []
                        },
                        "$xmlns:kml": namespaces.kml,
                        "$xmlns:xsi": namespaces.xsi,
                        "$xsi:schemaLocation": namespaces.schemata.kmlService,
                    }
                };
                ...
                switch (options.csvFormat) {
                    default:
                        ...
                        aux = ["Waypoint #" + i, "Waypoint exported using GIScience Universität Heidelberg OpenRouteService",
                            namespaces.metadata.authorName,
                            namespaces.metadata.authorEmailId + "@" + namespaces.metadata.authorEmailDomain,
                            namespaces.metadata.copyright,
                            namespaces.metadata.link,
                            date.toISOString(),
                            namespaces.metadata.keywords, (routeVars.routeCoords[i].lng).toFixed(coordinatePrecision), (routeVars.routeCoords[i].lat).toFixed(coordinatePrecision)
                        ];
                    ...
                    case 'yx':
                            ...
                            aux = ["Waypoint #" + i, "Waypoint exported using GIScience Universität Heidelberg OpenRouteService",
                                ...
                                namespaces.metadata.link,
                                date.toISOString(),
                                namespaces.metadata.keywords, (routeVars.routeCoords[i].lat).toFixed(coordinatePrecision), (routeVars.routeCoords[i].lng).toFixed(coordinatePrecision)
                            ];
                            csvBody[i] = aux.join(sep);
                        }
                        csvBody = csvBody.join('\r\n');
                    ...
                    case 'wkt':
                        ...
                        aux = [
                            ...
                            namespaces.metadata.description,
                            namespaces.metadata.authorName,
                            namespaces.metadata.authorEmailId + "@" + namespaces.metadata.authorEmailDomain,
                            namespaces.metadata.copyright,
                            namespaces.metadata.link,
                            date.toISOString(),
                            namespaces.metadata.keywords, (routeVars.routeGeometry).toUpperCase() + "(" + LatLngAux.join(',') + ")"
                        ];
                    ...
                    case 'wktWaypoints':
                            ...
                            aux = ["Waypoint #" + i, "Waypoint exported using GIScience Universität Heidelberg OpenRouteService",
                                ...
                                namespaces.metadata.link,
                                date.toISOString(),
                                namespaces.metadata.keywords, "POINT(" + (routeVars.routeCoords[i].lat).toFixed(coordinatePrecision) + ", " + (routeVars.routeCoords[i].lng).toFixed(coordinatePrecision) + ")"
                            ];
                            csvBody[i] = aux.join(sep);
                        }
                        csvBody = csvBody.join('\r\n');
    ...
    **/
    .factory('orsExportFactory', ['FileSaver', 'Blob', 'orsWriterFactory',
        function(FileSaver, Blob, orsWriterFactory) {
             ...
             * @param {Object} route: the route object (leaflet object) to export
