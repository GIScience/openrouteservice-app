    ...
    return {
        ...
        controller: ['$scope', '$filter', '$compile', '$timeout', 'orsSettingsFactory', 'orsLocationsService', 'orsObjectsFactory', 'orsRequestService', 'orsUtilsService', 'orsMapFactory', 'orsCookiesFactory', 'lists', 'globals', 'mappings', 'orsNamespaces', ($scope, $filter, $compile, $timeout, orsSettingsFactory, orsLocationsService, orsObjectsFactory, orsRequestService, orsUtilsService, orsMapFactory, orsCookiesFactory, lists, globals, mappings, orsNamespaces) => {
            ...
            $scope.geofeatures = {
                ...
                layerEmergencyBoundary: L.featureGroup(),
            ...
            $scope.boundarySwitcher.onAdd = function(map) {
                var div = $compile('<ors-emergency-boundary></ors-emergency-boundary>')($scope)[0];
                L.DomEvent.disableClickPropagation(div);
                return div;
            };
            $timeout(function() {
                $scope.mapModel.map.addControl($scope.boundarySwitcher);
            }, 500);
            $scope.switchRegions = () => {
                if ($scope.emergencyBoundaryShow == true) {
                    L.geoJSON(lists.boundary, {
                        style: lists.layerStyles.boundary()
                    }).addTo($scope.geofeatures.layerEmergencyBoundary);
                } else {
                    $scope.geofeatures.layerEmergencyBoundary.clearLayers();
                }
            };
            ...
            $scope.mapModel.map.on("load", (evt) => {
                ...
                $scope.mapModel.geofeatures.layerEmergencyBoundary.addTo($scope.mapModel.map);
                // add layer control
...
angular.module('orsApp').directive('orsEmergencyBoundary', ['$compile', '$timeout', 'orsSettingsFactory', ($compile, $timeout, orsSettingsFactory) => {
    return {
        restrict: 'E',
        template: `
                <div ng-attr-class="{{ 'ui message ors-map-message fade blue' }}">
                    <label>Datenbereich </label>
                      <span>
                        <input type="checkbox" ng-model="emergencyBoundaryShow" ng-change="switchRegions()" name="emergency-boundary">
                      </span>
                      <div>
                      	(Landesgrenze Schwaben)
                      </div>
                </div>`,
        link: (scope, elem, attr) => {}
    };
